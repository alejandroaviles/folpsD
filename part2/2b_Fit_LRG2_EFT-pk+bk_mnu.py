...
chain_name = (
    f"{base_dir}/cxxx_mocks_chains/c001/base/Pk_Bk_bugfixed/{tracers_str}"
...
chain_name= f'./chains_c2p/c2p_width5'
chain_name= f'{base_dir}/Abacus2gen_chains/mnu/{tracers_str}_{model}_{"std" if prior_basis == "standard" else prior_basis}_kr{kr_max:.3f}_kb0{kr_b0_max:.3f}_kb2{kr_b2_max:.3f}_{pt_model}_{"Afull" if A_full_status else "Ano"}'
...
def make_params(prior_basis, width_EFT, width_SN0, width_SN2,pt_model='folpsD',b3_coev=True,sigma8_fid=None):
    ...
    if prior_basis in ('physical', 'physical_prior_doc'):
        ...
        # params['c1p'] = {'prior': {'dist': 'norm', 'loc': 66.6, 'scale': 66.6*4}}
        # params['c2p'] = {'prior': {'dist': 'norm', 'loc': 0, 'scale': 1*4}}
        # params['Pshotp'] ={'prior': {'dist': 'norm', 'loc': 0, 'scale': Ppoisson*4}}
        # params['Bshotp'] = {'prior': {'dist': 'norm', 'loc': 0, 'scale': Ppoisson*4}}
        ...
        if b3_coev:
            ...
            params['b3'] = {'prior': {'dist': 'norm', 'loc': 0, 'scale': 5},'fixed':False}#TBD
        # params['b3'] = {'fixed':True}
    ...
    for tracer in tracers:
        for comp in ["ps",'bs']:  # handle PS and BS separately
            ...
            if comp=='ps':            
                ...
                emulator_filename = f'./Emulator_{comp}/{comp}_emu_{tracer}_z{z}_{kr_max}_Afull_{A_full_status}_hn.npy'
            else: 
                kr_b2_max_label= kr_b2_max if kr_b2_max>0.08 else 0.08
                emulator_filename = f'{base_dir}/Abacus2gen_chains/Emulator_{comp}_pkbk/{comp}_emu_{tracer}_z{z}_{kr_max}_{kr_b0_max}_{kr_b2_max_label}_Afull_{A_full_status}_hn.npy'
                emulator_filename = f'{base_dir}/Abacus2gen_chains/Emulator_{comp}_pkbk/{comp}_emu_{tracer}_z{z}_{kr_max}_{kr_b0_max}_{kr_b2_max_label}_Afull_{A_full_status}_hn.npy'
