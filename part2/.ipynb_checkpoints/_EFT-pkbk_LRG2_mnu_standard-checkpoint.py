...
base_dir = '/global/cfs/cdirs/desicollab/users/hernannb/__FOLPS_tutorial/scripts/chains'
...
chain_name = (
    f"{base_dir}/Abacus2gen_chains/mnu/_priordoc_{tracers_str}"
    ...
    f"{f'_damping_{damping}' if damping != 'lor' else ''}_planck_ns_centre"
)
...
def make_params(prior_basis, width_EFT, width_SN0, width_SN2,pt_model='folpsD',b3_coev=True,sigma8_fid=None,b1_fid=None):
    ...
    if prior_basis in ('physical', 'physical_prior_doc'):
        ...
        params['bsp'] = {'prior': {'dist': 'norm', 'loc': -2/7*(b1_fid-1)*(sigma8_fid)**2, 'scale': 5}}
        # params['bsp'] = {'prior': {'dist':'uniform','limits': [-50,50]}}
        if b3_coev:
            params['b3p'] = {'fixed':True}
        else:
            params['b3p'] = {'prior': {'dist': 'norm', 'loc': 23/42*(b1_fid-1)*(sigma8_fid)**3, 'scale': 1*(sigma8_fid)**3}}
        # PS-only
        ...
        params['c2p'] = {'prior': {'dist': 'norm', 'loc': 0, 'scale': 0.1}}
        ...
        if pt_model=='EFT':
            ...
            params['X_FoG_p'] = {'prior': {'dist':'uniform','limits': [0, 20]}} 
            params['X_FoG_b'] = {'prior': {'dist':'uniform','limits': [0, 20]}}
...
for tracer in tracers:
    ...
    b1_fid = all_tracer_b1_fid[tracer]
    # PS theory selection
    ...
    # --- Update parameters ---
    params = make_params(prior_basis, width_EFT, width_SN0, width_SN2, pt_model=pt_model,b3_coev=b3_coev,sigma8_fid=sigma8_fid,b1_fid=b1_fid)
...
def load_data_vector_and_covariance(tracer='LRG2',z_ev=0.8,k_max=0.301,k_max_b0=None,k_max_b2=None,P4=False):
    ...
    # np.savez("EFT_LRG2_Pk_0.201_Bk_0.12_0.08.npz",
    #      data=data,
    #      cov_array=cov_array,
    #      kr_pk=kr_pk,
    #      kr_b0=kr_b0,
    ...
    #      kb_all=kb_all,
    #      m_bin=m_bin,
    #      k_points_pk=k_points_pk,
    #      k_points_b0=k_points_b0,
    #      k_points_b2=k_points_b2)
...
observables = {}
for tracer in tracers:
    ps_obs, bs_obs, cov_array =  create_observable(["ps","bs"],tracer,tracer_redshifts[tracer],kr_max,kr_b0_max,kr_b2_max,hexa)
    observables[tracer] = {
        "ps": ps_obs,
        "bs": bs_obs,
        "cov_array": cov_array
...
if set_emulator:
    for tracer in tracers:
        for comp in ["ps",'bs']:  # handle PS and BS separately
            ...
            if comp=='ps':            
                emulator_filename = f'{base_dir}/Emulator_{comp}/{comp}_emu_{tracer}_z{z}_{kr_max}_Afull_{A_full_status}_{model}_{prior_basis}.npy'
            else: 
                kr_b2_max_label= kr_b2_max if kr_b2_max>0.08 else 0.08
                emulator_filename = f'{base_dir}/Emulator_{comp}_pkbk/{comp}_emu_{tracer}_z{z}_{kr_max}_{kr_b0_max}_{kr_b2_max_label}_Afull_{A_full_status}_{model}_{prior_basis}.npy'
