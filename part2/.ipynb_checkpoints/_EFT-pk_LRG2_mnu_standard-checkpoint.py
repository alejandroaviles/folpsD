...
chain_name = (
    ...
    f"_{'std' if prior_basis == 'standard' else 'priordoc'}"
    f"_kr{kr_max:.3f}"
    f"{f'_kb0{kr_b0_max:.3f}_kb2{kr_b2_max:.3f}' if bispectrum else ''}"
    f"{'_hexa' if hexa else ''}"
    f"_{pt_model}"
    # f"_{'Afull' if A_full_status else 'Ano'}"
    f"_{'b3coev' if b3_coev else 'b3free'}"
    f"{f'_damping-{damping}' if damping != 'lor' else ''}"
)
...
def make_params(prior_basis, width_EFT, width_SN0, width_SN2,pt_model='folpsD',b3_coev=True,sigma8_fid=None,b1_fid=None):
    ...
    if prior_basis in ('physical', 'physical_prior_doc'):
        ...
        if b3_coev:
            ...
            params['b3p'] = {'prior': {'dist': 'norm', 'loc': 23/42*(b1_fid-1)*(sigma8_fid)**4, 'scale': 1*(sigma8_fid)**4}}
        # PS-only
