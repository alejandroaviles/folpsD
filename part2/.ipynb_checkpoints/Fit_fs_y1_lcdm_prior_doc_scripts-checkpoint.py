...
prior_basis = 'physical_prior_doc' 
...
chain_name = f'/global/cfs/cdirs/desicollab/users/prakharb/folpsD_paper_chains/y1_chains/base/correct_measurements/MCMC-fs_klim_0-0.02-{k_max}_2-0.02-{k_max}-{tracers_str}-GCcomb_schoneberg2024-bbn_planck2018-ns10_{prior_basis}-prior_{model}_{pt_model}_Ano_int_freedom_TCM_full_Aap'
# chain_name= f'chains/test_folpsD_y1_prior_doc_priors_correct'
# if profiler:
profiler_chain_name = f'/global/cfs/cdirs/desicollab/users/prakharb/folpsD_paper_chains/y1_chains_profiler/base/Iminuit-fs_klim_0-0.02-{k_max}_2-0.02-{k_max}-{tracers_str}-GCcomb_schoneberg2024-bbn_planck2018-ns10_{prior_basis}-prior_{model}_{pt_model}_Ano_int_freedom_TCM_full_Aap'
...
for tracer in tracers:
    ...
    tracer_name_alias='LRG' if tracer in ('LRG1','LRG2','LRG3') else tracer
    theory = FOLPSv2TracerPowerSpectrumMultipoles(tracer=tracer_name_alias,template=template, prior_basis=prior_basis,A_full=False, b3_coev=False, sigma8_fid=sigma8_fid,h_fid=fiducial.h)

    #Update bias and EFT priors
    #theory.params['bs'].update(fixed=True)
    if prior_basis in ('physical', 'physical_prior_doc'):
        theory.params['b1p'].update(prior = {'dist':'uniform','limits': [1e-1,4]})
        theory.params['b2p'].update(prior = {'dist': 'norm', 'loc': 0, 'scale': 5})
        theory.params['bsp'].update(prior = {'dist': 'norm', 'loc': -2/7*sigma8_fid**2, 'scale': 5})
        # theory.params['bsp'].update(fixed=True,value= -2/7*sigma8_fid)
        theory.params['b3p'].update(prior = {'dist': 'norm', 'loc': 23/42*sigma8_fid**4, 'scale': 1*sigma8_fid**4},fixed=False)
        # theory.params['b3p'].update(fixed=True)
        # theory.params['b3p'].update(fixed=True,value= 23/42*sigma8_fid)
...
if set_emulator:
    ...
    for i in range(len(theories)):
        emulator_filename = f'/global/cfs/cdirs/desicollab/users/prakharb/folpsD_paper_chains/Emulator_y1_latest/emu_-{tracers_str}_{str(all_tracers[i])}.npy'
        # emulator_filename= f'masti'
...
print("likelihood: ",likelihood(),likelihood.varied_params)
...
# profiler = MinuitProfiler(likelihood, seed=42,save_fn=profiler_chain_name)
