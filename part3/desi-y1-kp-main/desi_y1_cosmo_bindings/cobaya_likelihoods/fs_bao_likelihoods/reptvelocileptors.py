...
class reptvelocileptors(Theory):
    ...
    def must_provide(self, **requirements):
        ...
        self.pk_dd_interpolator_fid = self.fiducial.get_fourier().pk_interpolator(of='delta_cb').to_1d(z=self.z)
        self.filter = PowerSpectrumBAOFilter(self.pk_dd_interpolator_fid, engine='peakaverage', cosmo=self.fiducial, cosmo_fid=self.fiducial)
        toret = {}
        toret['Pk_grid'] = {}
        toret['Pk_grid']['nonlinear'] = False
        toret['Pk_grid']['z'] = self.z
        toret['Pk_grid']['k_max'] = 1e2
        toret['Pk_grid']['vars_pairs'] = [('delta_nonu', 'delta_nonu'), ('v_newtonian_cdm', 'v_newtonian_cdm'), ('v_newtonian_cdm', 'v_newtonian_baryon'), ('v_newtonian_baryon', 'v_newtonian_baryon')]
        toret['Hubble'] = {'z': np.concatenate([[0.], self.z])}
        #require_f = self.z
        #toret['fsigma8'] = {'z': require_f}
        #toret['sigma8_z'] = {'z': self.z}
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        pk_dd = pk_dd_interpolator(self.kin)
        ...
        self.filter(pk_dd_interpolator, cosmo=cosmo)
        sigma8 = pk_dd_interpolator.sigma8()
        pknow_dd = self.filter.smooth_pk_interpolator()(self.kin)
        ...
        fsigma8 = pk_tt_interpolator.sigma8()
        #k0 = 1e-3
        #f0 = (pk_tt_interpolator(k0) / pk_dd_interpolator(k0))**0.5
        pk_tt = pk_tt_interpolator(self.kin)
    ...
    def get_pkpoles(self, params, z=None, sigv=15., fsat=0.1, sn=1e4, return_gradient=False):
        ...
        toret = tablevel_combine_bias_terms_poles(pktable, pars)
        if return_gradient:
            return toret, pktable[..., -7:].dot(gradient)
        return toret