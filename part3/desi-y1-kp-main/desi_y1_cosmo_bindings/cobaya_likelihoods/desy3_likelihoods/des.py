...
class DES(DataSetLikelihood):
    ...
    def get_requirements(self):
        return {
            ...
            "omegam": None,
            "Pk_interpolator": {
                "z": self.zs_interp, "k_max": 15 * self.acc, "nonlinear": (False, True),
    ...
    def get_theory(self, PKdelta_NL, PKdelta_L, PKWeyl, PKMW, bin_bias, shear_calibration_parameters,
        ...
        D_growth = PKdelta_L.P(self.zs, 0.001)
        D_growth = np.sqrt(D_growth / PKdelta_L.P(0, 0.001))
        ...
        if any(t in self.used_types for t in ["gammat", "wtheta"]):
            qgal = []
            qgalm = np.empty((self.nwbins, len(chis)))
            qgalm_b = np.empty(chis.shape)
            for b in range(self.nwbins):
                qgal.append([])
                zshift = self.zs - lens_photoz_errors[b]
                zbin = self.zbin_w_sp[b](zshift)
                n_chi = Hs * zbin/(sum(zbin)*0.01)
                n_chi[zshift < 0] = 0
                ...
                qgalm[b] = 3 * omegam * h2 * (1e5 / c) ** 2 * chis * (1 + self.zs) / 2 * qgalm_b * Ci[b]
        ...
        if numba:
            ...
            #non-Limber
            tmpnonlimber = PKdelta_NL.P(self.zs, ks, grid = False) - PKdelta_L.P(self.zs, ks, grid = False)
            _limber_PK_terms(tmpnonlimber, ks, dchifac, PKdelta_NL.kmax)
            #Limber
            tmp = PKdelta_NL.P(self.zs, ks, grid = False)
            _limber_PK_terms(tmp, ks, dchifac, PKdelta_NL.kmax)
        else:
            ...
            for ix, l in enumerate(ls_cl):
                ...
                weight[:] = dchifac
                weight[k < 1e-4] = 0
                weight[k >= PKdelta_NL.kmax] = 0
                #non-Limber
                tmpnonlimber[ix, :] = weight * (PKdelta_NL.P(self.zs, k, grid = False) - PKdelta_L.P(self.zs, k, grid = False))
                #Limber
                tmp[ix, :] = weight * PKdelta_NL.P(self.zs, k, grid = False)

        if PKWeyl is not None:
            if numba:
                #Limber
                tmplens = (PKdelta_NL.P(self.zs, ks, grid=False)/PKdelta_L.P(self.zs, ks, grid=False)) * PKWeyl.P(self.zs, ks, grid=False)
                _limber_PK_terms(tmplens, ks, dchifac, PKWeyl.kmax)
                tmpmw = -(PKdelta_NL.P(self.zs, ks, grid=False)/PKdelta_L.P(self.zs, ks, grid=False)) * PKMW.P(self.zs, ks, grid=False)
                _limber_PK_terms(tmpmw, ks, dchifac, PKMW.kmax)
            else:
                ...
                for ix, l in enumerate(ls_cl):
                    ...
                    #Limber
                    tmplens[ix, :] = weight * (PKdelta_NL.P(self.zs, k, grid=False)/PKdelta_L.P(self.zs, k, grid=False)) * PKWeyl.P(self.zs, k, grid=False)
                for ix, l in enumerate(ls_cl):
                    k = (l + 0.5) / chis
                    weight[:] = dchifac
                    weight[k < 1e-4] = 0
                    weight[k >= PKMW.kmax] = 0
                    #Limber
                    tmpmw[ix, :] = -weight * (PKdelta_NL.P(self.zs, k, grid=False)/PKdelta_L.P(self.zs, k, grid=False)) * PKMW.P(self.zs, k, grid=False)
        else:
            tmplens = tmp
            tmpmw = tmp
        ...
        if self.use_hankel:  # pragma: no cover
            ...
            if 'gammat' in self.used_types:
                for (f1, f2) in self.bin_pairs[self.data_types.index('gammat')]:
                    cl = _spline(ls_cl, np.dot(tmpmw, qgal[f1] * qs[f2]))
            ...
            if 'gammat' in self.used_types:
                for (f1, f2) in self.bin_pairs[self.data_types.index('gammat')]:
                    cl_limberall = np.sum(tmpmw * (np.outer(ls_cl*(ls_cl+1)/(ls_cl+0.5)**2, qgalm[f1]) + qgal[f1]) * qs[f2], axis=1)
                    cl = _spline(ls_cl, cl_limberall)(ls_bessel)
                    corrs_th_t[f1, f2] = np.dot(cl, j2s) * (1 + shear_calibration_parameters[f2])
            if 'wtheta' in self.used_types:
                for (f1, f2) in self.bin_pairs[self.data_types.index('wtheta')]:
                    cl_limberall = np.sum(tmp * (np.outer(ls_cl*(ls_cl+1)/(ls_cl+0.5)**2, qgalm[f1]) + qgal[f1]) * (np.outer(ls_cl*(ls_cl+1)/(ls_cl+0.5)**2, qgalm[f2]) + qgal[f2]), axis=1)
                    cl_limberfront = np.sum(tmpnonlimber * (np.outer(ls_cl*(ls_cl+1)/(ls_cl+0.5)**2, qgalm[f1]) + qgal[f1]) * (np.outer(ls_cl*(ls_cl+1)/(ls_cl+0.5)**2, qgalm[f2]) + qgal[f2]), axis=1)
                    cl_limber = cl_limberall[ls_cl>=200]
                    cl_front = cl_limberfront[ls_cl<200]          
                    cl_exact = cl_front + self.nonLimber(ls_exact, qgal[f1], qgal[f2], chis, D_growth, PKdelta_L,f_of_chi_spline, bin_bias[f1], bin_bias[f2], qgalm[f1], qgalm[f2])
    ...
    def nonLimber(self, ells, kernel1_vals, kernel2_vals, chis, D_growth, PK, f_interp=None, bias1=None, bias2=None, mkernel1=None, mkernel2=None):
        chi_pad_upper=1.
        chi_pad_lower=1.
        chi_extrap_upper=1.
        chi_extrap_lower=1.
        ...
        if f_interp is not None:
            if bias1 !=0 and bias2 != 0:
                ...
                w1_rsd = w1 * f_vals
                w2_rsd = w2 * f_vals
        if mkernel1 is not None:
            w1m = mkernel1 * D_growth/chis
            w1_mag = _spline(chis, w1m)(chi_vals)
        if mkernel2 is not None:
            w2m = mkernel2 * D_growth/chis
            w2_mag = _spline(chis, w2m)(chi_vals)
        
        cell = np.zeros_like(ells)
        for i_ell, ell in enumerate(ells):
            k_vals, I_1 = self.FFT(chi_vals, w1, ell, N_extrap_upper, N_extrap_lower, N_pad)
            k_vals, I_2 = self.FFT(chi_vals, w2, ell, N_extrap_upper, N_extrap_lower, N_pad)
            if f_interp is not None:
                k_vals_check, I_1_rsd = self.FFT(chi_vals, w1_rsd, ell, N_extrap_upper, N_extrap_lower, N_pad, nu = 1.1)
                k_vals_check, I_2_rsd = self.FFT(chi_vals, w2_rsd, ell, N_extrap_upper, N_extrap_lower, N_pad, nu = 1.1)
                assert np.allclose(k_vals_check, k_vals)
                I_1 = I_1 - I_1_rsd
                I_2 = I_2 - I_2_rsd
            if mkernel1 is not None:
                k_vals_check2, I_1_m = self.FFT(chi_vals, w1_mag, ell, N_extrap_upper, N_extrap_lower, N_pad)
                assert np.allclose(k_vals_check2, k_vals)
                I_1 = I_1 + (ell*(ell+1)/k_vals**2)*I_1_m
            if mkernel2 is not None:
                k_vals_check2, I_2_m = self.FFT(chi_vals, w2_mag, ell, N_extrap_upper, N_extrap_lower, N_pad)
                assert np.allclose(k_vals_check2, k_vals)
                I_2 = I_2 + (ell*(ell+1)/k_vals**2)*I_2_m
            logk_vals = np.log(k_vals)
            pk_vals = PK.P(0, k_vals)
    ...
    def FFT(self, x, fx, ell, N_extrap_high = 0, N_extrap_low = 0, N_pad = 0, nu = 1.0):
        dlnx = np.log(x[1]/x[0])
        c_window_width = 0.25

        # extrapolate x and f(x) linearly in log(x), and log(f(x))
        x = self.log_extrap(x, N_extrap_low, N_extrap_high)
        fx = self.log_extrap(fx, N_extrap_low, N_extrap_high)
        N = np.size(x)

        # zero-padding
        if(N_pad):
            pad = np.zeros(N_pad)
            x = self.log_extrap(x, N_pad, N_pad)
        ...
        c_m=rfft(f_b)
        m=np.arange(0,N//2+1) 
        c_m = c_m * self.c_window(m, int(c_window_width*N//2.) )
        eta_m = 2*np.pi/(float(N)*dlnx) * m
        
        z_ar = nu + 1j*eta_m
        y = (ell+1.) / x[::-1]
        if nu ==1:
            h_m = c_m * (x[0]*y[0])**(-1j*eta_m) * self.g_l(ell, z_ar)
        else:
            h_m = c_m * (x[0]*y[0])**(-1j*eta_m) * self.g_l_2(ell, z_ar)

        Fy = irfft(np.conj(h_m)) * y**(-nu) * np.sqrt(np.pi)/4.
    ...
    def g_l(self, l, z_array):
        gl = 2.**z_array * self.g_m_vals(l+0.5,z_array-1.5)
        return gl

    def g_l_2(self, l,z_array):
        gl2 = 2.**(z_array-2) *(z_array -1)*(z_array -2)* self.g_m_vals(l+0.5,z_array-3.5)
        return gl2

    def g_m_vals(self, mu, q):
        imag_q= np.imag(q)
    ...
    def logp(self, **params_values):
        PKdelta_L = self.provider.get_Pk_interpolator(("delta_tot", "delta_tot"), nonlinear=False,
                                                    extrap_kmax=3000 * self.acc)
        PKdelta_NL = self.provider.get_Pk_interpolator(("delta_tot", "delta_tot"), nonlinear=True,
                                                    extrap_kmax=3000 * self.acc)
        if self.use_Weyl:
            PKWeyl = self.provider.get_Pk_interpolator(("Weyl", "Weyl"), nonlinear=False,
                                                       extrap_kmax=3000 * self.acc)
            PKMW = self.provider.get_Pk_interpolator(("delta_tot", "Weyl"), nonlinear=False,
                                                       extrap_kmax=3000 * self.acc)
        else:
            PKWeyl = None
            PKMW = None
        ...
        lens_photoz_errors = [
            params_values.get(p, None) for p in
            ['DES_DzL1', 'DES_DzL2', 'DES_DzL3', 'DES_DzL4', 'DES_DzL5', 'DES_DzL6']]
        bin_bias = [
            params_values.get(p, None) for p in
            ['DES_b1', 'DES_b2', 'DES_b3', 'DES_b4', 'DES_b5', 'DES_b6']]
        shear_calibration_parameters = [
            params_values.get(p, None) for p in
            ['DES_m1', 'DES_m2', 'DES_m3', 'DES_m4']]
        theory = self.get_theory(
            PKdelta_NL, PKdelta_L, PKWeyl, PKMW,
            bin_bias=bin_bias,
