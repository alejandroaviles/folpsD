"""
.. module:: theories.isitgr

:Synopsis: Managing the ISiTGR cosmological code
...
This module imports and manages the ISiTGR cosmological code.
It requires ISiTGR 1.0.3 or higher.
...
class ISiTGR(BoltzmannBase):
    ...
    _camb_repo_version = os.environ.get("ISiTGR_REPO_VERSION", "master")
    _camb_min_gcc_version = "6.4"
    _min_camb_version = '1.0.3'

    file_base_name = 'isitgr'
    ...
    def initialize(self):
        ...
        try:
            ...
            self.camb = load_external_module(
                "isitgr", path=self.path, install_path=install_path,
    ...
    def get_import_path(path):
        ...
        """
        lib_fname = "isitgrdll.dll" if platform.system() == "Windows" else "isitgrlib.so"
        if not os.path.isfile(os.path.realpath(os.path.join(path, "isitgr", lib_fname))):
