...
tm_recon = tm.clone(scheduler=dict(max_workers=10), provider=dict(provider='nersc', time='03:00:00', mpiprocs_per_worker=64, output=output, error=error, kwargs=kwargs))
tm_combine = tm.clone(scheduler=dict(max_workers=10), provider=dict(provider='local'))
...
if __name__ == '__main__':
    ...
    from desi_y1_files import get_ez_file_manager, get_data_file_manager, get_box_ez_file_manager, is_baseline_2pt_setup, get_zsnap_from_z
    ...
    def compute_merged_region(fi):
        ...
        data = fm.get(id='catalog_data_merged_ez_y1', **options, ignore=True)
        all_randoms = fm.select(id='catalog_randoms_merged_ez_y1', **options, ignore=True)
        kwargs = {}
        kwargs['output_wmatrix'] = fm.get('wmatrix_power_merged_ez_y1', **options, ignore=True)
        kwargs['output_window'] = fm.select('window_power_merged_ez_y1', **options, ignore=True)
        ...
        kwargs['output_wmatrix_nodirect'] = fm.get('wmatrix_power_merged_ez_y1', **{**options, 'cut': [None]}, ignore=True)
    ...
    def compute_region(fi):
        ...
        data = fm.get(id='catalog_data_ez_y1', **options, ignore=True)
        if not data.exists(): return None
        if fi.exists(): return fi
        all_randoms = fm.select(id='catalog_randoms_ez_y1', **options, ignore=True)
        if not all(randoms.exists() for randoms in all_randoms): return None
        kwargs = {}
        if recon:
            data_recon = fm.get(id='catalog_data_recon_ez_y1', **options, ignore=True)
            randoms_recon = fm.select(id='catalog_randoms_recon_ez_y1', **options, ignore=True)
            data_recon_modes = [fm.get(id='catalog_data_recon_ez_y1', **{**options, 'mode': mode}, ignore=True) for mode in ['recsym', 'reciso']]
            all_randoms_recon_modes = [fm.select(id='catalog_randoms_recon_ez_y1', **{**options, 'mode': mode}, ignore=True) for mode in ['recsym', 'reciso']]
            run_recon = not data_recon.exists() or not all(ff.exists() for ff in randoms_recon)
            if run_recon:
    ...
    for stat_type in todo:
        if stat_type == 'symlink':
            ...
            for id in ['correlation_ez_y1', 'correlation_recon_ez_y1', 'power_ez_y1', 'power_recon_ez_y1', 'wmatrix_power_merged_ez_y1', 'wmatrix_power_merged_rotated_ez_y1']:
            ...
            for id in ['catalog_data_recon_ez_y1', 'catalog_randoms_recon_ez_y1']:
        ...
        if stat_type == 'merge_catalogs':
            for fo in fm.select(id='catalog_data_merged_ez_y1', region=['NGC', 'SGC'], tracer=tracers, version=version, fa=fa).iter(intersection=False):
                fi = fm.select(id='catalog_data_ez_y1', **fo.options, ignore=True, imock=imock)
                merge_catalogs(fo, fi, factor=100.)

            for fo in fm.select(id='catalog_randoms_merged_ez_y1', region=['NGC', 'SGC'], tracer=tracers, version=version, fa=fa).iter(intersection=False):
                fi = fm.select(id='catalog_randoms_ez_y1', **fo.options, ignore=True, imock=imock)
                merge_randoms_catalogs(fo, fi)
                
        if stat_type == 'merge':
            for fi in fm.select(id='power_merged_ez_y1', region=regions, tracer=tracers, version=version, fa=fa, weighting=weighting, cut=cut).iter(intersection=False):
                fid = fi.id
                options = fi.options
                if options['region'] == 'GCcomb':
                    fis = [compute_merged_region(fi) for fi in fm.select(id=fid, **{**options, 'region': ['NGC', 'SGC']})]
                    inputs_wmatrix = [[fm.get(id='wmatrix_power_merged_ez_y1', **{**options, 'region': region, 'cut': [cut]}, ignore=True, raise_error=False) for cut in [cut, None]] for region in ['NGC', 'SGC']]
                    output_wmatrix = [fm.get(id='wmatrix_power_merged_ez_y1', **{**options, 'region': 'GCcomb', 'cut': [cut]}, ignore=True, raise_error=False) for cut in [cut, None]]
        ...
        if stat_type == 'ric_wmatrix':  # not used in the end
            dfm = get_data_file_manager()
            for fi in fm.select(id='ric_wmatrix_power_merged_ez_y1', version='v1', region=regions, tracer=tracers, cut=None, fa=fa, zrange=(0.8, 2.1)).iter(intersection=False):
                fid = fi.id
                ...
                print(options)
                wmatrix = fm.get(id='wmatrix_power_merged_ez_y1', version=version, **options, ignore=True)
                compute_ricwmatrix(fi, wmatrix, power_ric, power_noric, covariance=None)
                output_wmatrix = fm.get(id='wmatrix_ric_power_merged_ez_y1', version=version, **options, ignore=True)
                postprocess_ricwmatrix(fi, wmatrix, wmatrix_cut=wmatrix, output_wmatrix=output_wmatrix)
                wmatrix_cut = fm.get(id='wmatrix_power_merged_ez_y1', version=version, **{**options, 'cut': cut}, ignore=True)
                output_wmatrix_cut = fm.get(id='wmatrix_ric_power_merged_ez_y1', version=version, **{**options, 'cut': cut}, ignore=True)
        ...
        if stat_type == 'rotate_wmatrix':
            ...
            for ric in ['', '_ric'][:1]:
                for fi in fm.select(id='rotation_wmatrix{}_power_merged_ez_y1'.format(ric), region=regions, tracer=tracers, version=version, cut=[None, cut][1:], fa=fa).iter(intersection=False):
                    fid = fi.id
                    options = dict(fi.options)
                    #options.pop('version')
                    options.pop('nran')
                    fcovariance = dfm.get(id='covariance_y1', **{**options, 'source': 'thecov', 'observable': 'power', 'cut': None, 'version': 'v1.2'}, ignore=True)
                    data = [fm.get(id='power_ez_y1', **{**options, 'version': 'v1noric'}, imock=iimock, ignore=True) for iimock in range(1, 51)]
                    #data_rotated = [fm.get(id='power_rotated_ez_y1', **options, imock=iimock, ignore=True) for iimock in imock]
                    tracer = options['tracer']
                    zsnap = get_zsnap_from_z(tracer=tracer, z=options['zrange'])
                    print(tracer, zsnap)
                    theory = [tfm.get(id='power_box_ez_y1', version='v1', tracer=tracer, zsnap=zsnap, imock=iimock, los=los, ignore=True) for iimock in range(1, 11) for los in ['x', 'y', 'z']]
                    wmatrix = fm.get(id='wmatrix{}_power_merged_ez_y1'.format(ric), **options)
                    output_wmatrix = fm.get(id='wmatrix{}_power_merged_rotated_ez_y1'.format(ric), **options)
        ...
        for fi in fm.select(id=fid, region=regions, tracer=tracers, version=version, fa=fa, weighting=weighting, cut=cuts, imock=imock).iter(intersection=False):
            ...
            if options['cut'] is not None: continue
            if options['region'] == 'GCcomb':
