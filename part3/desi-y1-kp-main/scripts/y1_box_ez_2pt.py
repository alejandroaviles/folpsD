...
queue = Queue('y1_box_ez_2pt')
queue.clear(kill=False)

output, error = '_sbatch_box_ez/slurm-%j.out', '_sbatch_box_ez/slurm-%j.err'
...
if __name__ == '__main__':
    ...
    from desi_y1_files import get_box_ez_file_manager
    ...
    fm = get_box_ez_file_manager()
    fm.save('y1_box_ez_files.yaml', replace_environ=False)  # exports files as a *.yaml file
    
    imock = list(range(1, 1001))[:10]

    def compute_merged_region(fi):
        stat_type = fi.filetype
        options, fid = fi.options, fi.id
        compute = compute_power_spectrum_wmatrix
        data = list(fm.select(id='catalog_data_box_ez_y1', **options, imock=0, ignore=True))
        kwargs = {}
        kwargs['output_wmatrix'] = fm.get('wmatrix_power_merged_box_ez_y1', **options, ignore=True)
    ...
    def compute_region(fi):
        ...
        data = list(fm.select(id='catalog_data_box_ez_y1', **options, ignore=True))
        assert all(dd.exists() for dd in data)
        kwargs = {}
        if recon:
            data_recon = fm.get(id='catalog_data_recon_box_ez_y1', **options, ignore=True)
            data_recon_modes = [fm.get(id='catalog_data_recon_box_ez_y1', **{**options, 'mode': mode}, ignore=True) for mode in ['recsym', 'reciso']]
            all_randoms_recon_modes = [fm.select(id='catalog_randoms_recon_box_ez_y1', **{**options, 'mode': mode}, ignore=True) for mode in ['recsym', 'reciso']]
    ...
    for stat_type in todo:
        ...
        if stat_type == 'merge':
            for fi in fm.select(id='power_merged_box_ez_y1', tracer=tracers).iter(intersection=False):
                fid = fi.id
                options = fi.options
        ...
        fid = '{}_box_ez_y1'.format(stat_type)
