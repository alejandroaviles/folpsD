...
tm_power = tm.clone(scheduler=dict(max_workers=30), provider=dict(provider='nersc', time='00:30:00', mpiprocs_per_worker=64, nodes_per_worker=1, output=output, error=error))
tm_power_wmatrix = tm.clone(scheduler=dict(max_workers=10), provider=dict(provider='nersc', time='02:00:00', mpiprocs_per_worker=64, nodes_per_worker=1, output=output, error=error, stop_after=1))
gpu_nthreads = 4
cpu_nthreads = 64
gpu = True
if gpu:
    nthreads = gpu_nthreads
    tm_corr = tm.clone(scheduler=dict(max_workers=30), provider=dict(provider='nersc', time='00:40:00', mpiprocs_per_worker=1, output=output, error=error, constraint='gpu'))
else:
    nthreads = cpu_nthreads
    tm_corr = tm.clone(scheduler=dict(max_workers=40), provider=dict(provider='nersc', time='00:40:00', mpiprocs_per_worker=64, output=output, error=error))
...
if __name__ == '__main__':
    ...
    from desi_y1_files import get_raw_abacus_file_manager, get_data_file_manager, get_box_abacus_file_manager, is_baseline_2pt_setup, get_zsnap_from_z
    ...
    def compute_merged_region(fi):
        ...
        data = fm.get(id='catalog_data_merged_raw_abacus_y1', **{**options, 'region': 'ALL'}, ignore=True)
        #all_randoms = fm.select(id='catalog_randoms_merged_raw_abacus_y1', **{**options, 'region': 'ALL'}, ignore=True)
        all_randoms = fm.select(id='catalog_randoms_raw_abacus_y1', **{**options, 'region': 'ALL'}, ignore=True)
        kwargs = {}
        print(options)
        kwargs['output_wmatrix'] = fm.get('wmatrix_power_merged_raw_abacus_y1', **options, ignore=True)
        kwargs['output_window'] = fm.select('window_power_merged_raw_abacus_y1', **options, ignore=True)
        ...
        kwargs['output_wmatrix_nodirect'] = fm.get('wmatrix_power_merged_raw_abacus_y1', **{**options, 'cut': [None]}, ignore=True)
    ...
    def compute_region(fi):
        ...
        region = fi.options['region']
        region = 'ALL'
        data = fm.get(id='catalog_data_raw_abacus_y1', **{**options, 'region': region}, ignore=True)
        assert data.exists()
        all_randoms = fm.select(id='catalog_randoms_raw_abacus_y1', **{**options, 'region': region}, ignore=True)
        #print(options, fi, data, all_randoms[0])
        #print(fi)
        kwargs = {}
        if recon:
            data_recon = fm.get(id='catalog_data_recon_raw_abacus_y1', **options, ignore=True)
            randoms_recon = fm.select(id='catalog_randoms_recon_raw_abacus_y1', **options, ignore=True)
            data_recon_modes = [fm.get(id='catalog_data_recon_raw_abacus_y1', **{**options, 'mode': mode}, ignore=True) for mode in ['recsym', 'reciso']]
            all_randoms_recon_modes = [fm.select(id='catalog_randoms_recon_raw_abacus_y1', **{**options, 'mode': mode}, ignore=True) for mode in ['recsym', 'reciso']]
    ...
    for stat_type in todo:
        if stat_type == 'symlink':
            import os, glob
            for id in ['correlation_raw_abacus_y1', 'correlation_recon_raw_abacus_y1', 'power_raw_abacus_y1', 'power_recon_raw_abacus_y1', 'wmatrix_power_raw_abacus_y1']:
                for fi in fm.select(id=id, catalog=catalog, cut=[None, cut], version=version).iter(intersection=False):
            ...
            for id in ['catalog_data_recon_raw_abacus_y1', 'catalog_randoms_recon_raw_abacus_y1']:
        ...
        if stat_type == 'merge_catalogs':
            for fo in fm.select(id='catalog_data_merged_raw_abacus_y1', tracer=tracers, version=version, zshuffled=shuffled).iter(intersection=False):
                fi = fm.select(id='catalog_data_raw_abacus_y1', **fo.options, imock=imock, ignore=True)
                merge_catalogs(fo, fi, factor=50.)
            #for fo in fm.select(id='catalog_randoms_merged_raw_abacus_y1', tracer=tracers, version=version).iter(intersection=False):
            #    fi = fm.select(id='catalog_randoms_raw_abacus_y1', **fo.options, imock=imock)
            #    merge_randoms_catalogs(fo, fi)

        if stat_type == 'merge':
            for fi in fm.select(id='power_merged_raw_abacus_y1', region=regions, tracer=tracers, version=version, catalog=catalog, cut=cut, zshuffled=zshuffled, weighting='default_FKP', zrange=(0.6, 0.8)).iter(intersection=False):
                fid = fi.id
                options = fi.options
                if options['region'] == 'GCcomb':
                    fis = [compute_merged_region(fi) for fi in fm.select(id=fid, **{**options, 'region': ['NGC', 'SGC']})]
                    inputs_wmatrix = [[fm.get(id='wmatrix_power_merged_raw_abacus_y1', **{**options, 'region': region, 'cut': [cut]}, ignore=True, raise_error=False) for cut in [cut, None]] for region in ['NGC', 'SGC']]
                    output_wmatrix = [fm.get(id='wmatrix_power_merged_raw_abacus_y1', **{**options, 'region': 'GCcomb', 'cut': [cut]}, ignore=True, raise_error=False) for cut in [cut, None]]
        ...
        if stat_type == 'rotate_wmatrix':
            ...
            tfm = get_box_abacus_file_manager().select(version='v1.1')
            for fi in fm.select(id='wmatrix_power_merged_raw_abacus_y1', region=regions, tracer=tracers, version=version, catalog=catalog, cut=[None, cut]).iter(intersection=False):
                fid = fi.id
                options = fi.options
                fcovariance = dfm.get(id='covariance_power_y1', **{**options, 'cut': None, 'version': 'v0.6'}, ignore=True)
                data = [fm.get(id='power_raw_abacus_y1', **options, imock=iimock, ignore=True) for iimock in imock]
                tracer = options['tracer']
                zsnap = get_zsnap_from_z(tracer=tracer, z=options['zrange'])
                theory = [tfm.get(id='power_box_abacus_y1', tracer=tracer, zsnap=zsnap, imock=iimock, los=los, ignore=True) for iimock in imock for los in ['x', 'y', 'z']]
                output = fm.get(id='rotation_wmatrix_power_merged_raw_abacus_y1', **options)
                output_wmatrix = fm.get(id='wmatrix_power_merged_rotated_raw_abacus_y1', **options)
                #fi = fm.get(id='wmatrix_power_merged_raw_abacus_y1', tracer='QSO', zrange=(0.8, 2.1), version=version, region='GCcomb', cut=None)
                rotate_wmatrix(output, fi, fcovariance, theory=theory, data=data, output_wmatrix=output_wmatrix)

        recon = 'recon' in stat_type
        fid = '{}_raw_abacus_y1'.format(stat_type)
        stat_type = stat_type.split('_')[0]
        if recon: cuts = [None]
        elif 'power' in stat_type: cuts = [cut]  # no-cut P(k) inferred from cut one
        else: cuts = [None, cut]
        #cuts = [None]
        for fi in fm.select(id=fid, region=regions, tracer=tracers, version=version, catalog=catalog, weighting='default', zshuffled=zshuffled, cut=cuts, imock=imock, zrange=(0.6, 0.8)).iter(intersection=False):
            options = fi.options
            if options['region'] == 'GCcomb':
