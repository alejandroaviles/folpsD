...
if __name__ == '__main__':
    ...
    todo = ['bao', 'bao_recon', 'full_shape'][1:2]
    ...
    for fit_type in todo:
        ...
        for options in get_options(fit_type=fit_type):
            ...
            #options['split'] = None
            #options.pop('nran')
            fdata = fm.get(id='{}{}{}{}_y1'.format(observable, '_recon' if 'recon' in fit_type else '', rotated, '_corrected' if 'photo' in syst else ''), **options, ignore=True)
            print(fdata)
            continue
            ...
            #sample(fo, emulator_fn=femulator, resume=False, **kwargs)
